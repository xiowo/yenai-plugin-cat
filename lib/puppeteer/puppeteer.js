...
export default new class extends render {
  ...
  async launch() {
    ...
    logger.mark("[Yenai-Plugin-Cat]Puppeteer launching")
    // if (!puppeteer) {
    //   logger.error(`[Yenai-Plugin-Cat][Puppeteer]缺少依赖项，请执行 ${logger.red('pnpm add puppeteer-extra puppeteer-extra-plugin-stealth -w')}`)
    //   return false
    // }
    ...
    if (!this.browser) {
      logger.error("[Yenai-Plugin-Cat]puppeteer launching error")
      return false
    }
    logger.mark("[Yenai-Plugin-Cat]Puppeteer launched")
    /** 监听Chromium实例是否断开 */
    this.browser.on("disconnected", (e) => {
      logger.error("[Yenai-Plugin-Cat]Chromium实例关闭或崩溃！")
      this.browser = false
    })
    ...
    if (this.renderNum % this.restartNum === 0) {
      if (this.shoting.length <= 0) {
        setTimeout(async () => {
          ...
          this.browser = false
          logger.mark("[Yenai-Plugin-Cat]puppeteer 关闭重启...")
        }, 100)
      }
    }
  }
