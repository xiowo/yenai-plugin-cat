...
export default new class {
  ...
  async post(url, options = {}) {
    ...
    if (options.data) {
      logger.debug("[Yenai-Plugin-Cat]POST request params data: ", options.data)
  ...
  getAgent(cf, isProxy = true) {
    ...
    return cf
      ? this.getTlsVersionAgent(proxyAddress, isProxy)
      : isProxy
  ...
  async proxyRequestImg(url, { cache, timeout, headers } = {}) {
    ...
    logger.debug(`[Yenai-Plugin-Cat][proxyRequestImg][${_.truncate(url)}] ${kb} ${ms}`)
  ...
  _prepareRequest(options) {
    ...
    logger.debug(`[Yenai-Plugin-Cat] ${options.method.toUpperCase()}请求：${decodeURI(options.url)}`)
    options.headers = {
