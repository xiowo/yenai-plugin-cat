  ...
  return Promise.all(parsedData.list.map((site) => {
    ...
    return handleSite(site).finally(() => {
      if (--currentRequests === 0) {
        logger.debug("[Yenai-Plugin-Cat][状态]已完成所有网络测试")
      }
    })
  }))
}
...
const handleError = (error, siteName) => {
  let errorMsg = "Error"
  const prefix = "[Yenai-Plugin-Cat][状态]"
...
async function getNetworkLatency(url, timeoutTime = 5000, useProxy = false) {
  ...
  try {
    ...
    logger.debug(`[Yenai-Plugin-Cat][状态][网络测试][${url}] ${logger.blue(status)} ${logger.green(delay + "ms")}`)
