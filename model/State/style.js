...
export async function getBackground() {
  ...
  try {
    ...
    logger.info(`[Yenai-Plugin-Cat][状态]背景图片请求成功 ${fileSizeInKB}KB ${elapsedTime}ms`)
    ...
    backdrop && logger.warn(`[Yenai-Plugin-Cat][状态]背景图请求失败，使用默认背景图 “${bg}” ，错误原因: ${err.message}`)
    return bg
  } finally {
    clearTimeout()
  }
...
function getDefaultBackdrop(backdropDefault) {
  const Plugin_Path = "../../../../../plugins/yenai-plugin-cat"
  const Bg_Path = "./plugins/yenai-plugin-cat/resources/state/img/bg"
  if (backdropDefault === "random") {
    backdropDefault = _.sample(fs.readdirSync(Bg_Path))
    logger.debug(`[Yenai-Plugin-Cat][状态]使用随机背景图 “${backdropDefault}”`)
  }
